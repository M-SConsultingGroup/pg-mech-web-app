...
const SampleInvoice = () => {
	...
	const toggleAddOn = (addOnId: string) => {
		setItems(prevItems => {
			if (prevItems.some(item => item.id === addOnId)) {
				return prevItems.filter(item => item.id !== addOnId);
			} else {
	...
	const handleSave = async () => {
		...
		if (isSaving || items.length === 0 || !element) return;
	...
	return (
		<div className="min-h-screen p-4 bg-white print:p-0">
			<div id='invoice-page' className="max-w-5xl mx-auto bg-white p-6 rounded-lg shadow-lg print:shadow-none">
				{/* Invoice Header */}
				<div className="flex justify-between mb-8 border-b pb-4">
					...
					<div className="text-right">
						...
						<p>Invoice #: {customerInfo.invoiceNumber}</p>
						<p>LIC#TACLB51418C</p>
					</div>
				</div>
				...
				{/* Display customer info in print view */}
				<div className="mb-8 border-b pb-4 hidden print:block">
					...
					<p>{customerInfo.email || 'Customer Email'}</p>
					<p>{customerInfo.phone || 'Customer Phone'}</p>
				</div>
						...
						{addOns.map((addOn) => (
							<div key={addOn.id} className="flex items-center hover:bg-gray-50 rounded">
								<input
									...
									checked={items.some(item => item.id === addOn.id)}
									className="mr-2"
								/>
